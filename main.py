...
class Accumulator:
    ...
    def calculate(self, radius):
        """Calculates an approximation of pi using the Accumulator Approximations method"""
        self.drawCircle(radius)
        startTime = time.time()
        numInside = 0 # number of points that fall inside the circle
        for i in range(self.iterations):
            ...
            if math.sqrt(x**2 + y**2) <= radius:
                numInside += 1
                self.display(x, y, 1) # display a blue point inside the circle
            else:
                self.display(x, y, 0) # display a red point outside the circle
        # calculate pi approximation
        piEstimate = 4 * numInside / self.iterations
        endTime = time.time()
        turtle.resetscreen()
        turtle.reset()
        print(f"Accumulator Approximations method for estimating pi:\nNumber of points: {self.iterations}\nPi estimate: {piEstimate}\nTime: {endTime - startTime} seconds\n")
